...
class Teams(BaseModel, db.Model):
    ...
    def serialize(self) -> Dict[str, Any]:
        ...
        if serialized.get('training_time'): serialized['trainingTime'] = serialized.pop('training_time')
        if serialized.get('about_us'): serialized['aboutUs'] = serialized.pop('about_us')
        if serialized.get('is_mix_team'): serialized['isMixTeam'] = serialized.pop('is_mix_team')
        if serialized.get('last_tournament_played'): serialized['lastTournamentPlayed'] = serialized.pop('last_tournament_played')
        if serialized.get('last_tournament_organized'): serialized['lastTournamentOrganized'] = serialized.pop('last_tournament_organized')
